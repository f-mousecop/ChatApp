...
namespace ChatApp.Models.Auth
{
    ...
    public class DummyAuthServ : IAuthService
    {
        ...
        /// <returns><see cref="AuthResult.Ok(UserModel)"/> or <see cref="AuthResult.Fail(string)"/></returns>
        public Task<AuthResult> LoginAsync(string username, string password)
        {
            ...
            {
                return Task.FromResult(AuthResult.Ok(new UserModel { Id = 1, Username = username }));
            }
        ...
        public Task<AuthResult> RegisterAsync(string username, string password)
        {
            ...
            return Task.FromResult(AuthResult.Ok(new UserModel { Id = 2, Username = username }));
        }
    }
}
